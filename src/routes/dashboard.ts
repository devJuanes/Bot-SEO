...
import { getLeadStats, listRecentLeads, listRecentRuns } from '../db/leads.js';
import {
  countBlogPosts,
  countContentScripts,
  countOpportunities,
...
import { countConversations } from '../db/whatsapp.js';
  ...
  app.get('/api/dashboard', async (request) => {
    ...
    return withRequestTenant(request.tenant, async () => {
      const [
        ...
        listRecentRuns(25),
        getLeadStats(),
        countOpportunities().catch(() => 0),
        countBlogPosts().catch(() => 0),
        countContentScripts().catch(() => 0),
        countPendingBriefs().catch(() => 0),
        countConversations().catch(() => 0),
        listAppConnections(),
      ...
      return {
        ...
        stats: {
          leadsApprox: leadStats.total,
          needsWebsite: leadStats.needsWebsite,
          opportunities: opportunityCount,
          blogs: blogCount,
          scripts: scriptCount,
          apps: apps.length,
          pendingBriefs: briefCount,
          agentsEnabled: projectAgents.filter((a) => a.is_enabled).length,
          whatsappConversations: whatsappCount,
          byStatus: leadStats.byStatus,
          bySource: leadStats.bySource,
        },
        leads,
        opportunities: await listOpportunities(30).catch(() => []),
        blogs: await listBlogPosts(10).catch(() => []),
        scripts: await listContentScripts(10).catch(() => []),
        briefs: await listPendingBriefs(15).catch(() => []),
        apps,
        runs,
        logs: getLogs(100),
